...
const PickupForm = ({
    ...
    const handleSelect = (schedule) => {
        ...
        if (schedule === "no") updateFields({ IsScheduled: selectedSchedule, scheduledDate: {} })
    };

    const [value, setValue] = useState([timeToValue("10:00"), timeToValue("20:00")]);

    const handleChange = (event, newValue) => {
        setValue(newValue);
        console.log({ pickupTimeFrom: formatTime(value[0]), pickupTimeTo: formatTime(value[1]) });
    ...
    const [shipmentDate, setShipmentDate] = useState({ weekday: "Thursday", year: "", month: "May", day: "16" })
        ...
        <>
            <Modal show={isModalOpen} onClose={closeModal} >
                <LocalizationProvider dateAdapter={AdapterDayjs}>
                    <StaticDatePicker onClose={closeModal} onAccept={datechange} orientation='landscape' />
                </LocalizationProvider>
            </Modal>


            <FormWrapper title="Pickup Details">
                <div className="font-medium mb-4">Would you like to schedule you courier pickup?</div>
                <div className="flex my-4">
                    <label
                        className={`preset-container relative flex items-center justify-center p-3 rounded-lg cursor-pointer border-2 w-48 h-20 mr-3 ${IsScheduled === 'yes' ? 'selected bg-gray-700 text-white' : 'border-transparent bg-gray-300'
                            }`}
                    ...
                    >
                        ...
                        />
                        <div>
                            <div className="font-normal text-[1.1rem]">Yes - Schedule Pickup</div>
                        </div>

                        {
                            IsScheduled === 'yes' && <FaCheck className="absolute top-[70%] left-[85%]" />
                        }
                    </label>

                    <label
                        className={`preset-container relative flex items-center justify-center p-3 rounded-lg cursor-pointer border-2 w-48 h-20 ml-3 ${IsScheduled === 'no' ? 'selected bg-gray-700 text-white' : 'border-transparent bg-gray-300'
                            }`}
                    ...
                    >
                        ...
                        {
                            IsScheduled === 'no' && <FaCheck className="absolute top-[70%] left-[85%]" />
                        }
                    </label>
                </div>

                <div className={`${IsScheduled === "yes" ? "block" : IsScheduled === "no" ? " relative" : "hidden"}`}>
                    <div className={`${IsScheduled === "no" ? "absolute w-full h-full glass-morphism-overlay z-10" : "hidden"}`}></div>
                    <div className="charge-warning flex items-center my-8 overflow-hidden rounded-lg">
                        ...
                        <span className="px-2 font-semibold color-orange">Additional Charges may apply</span>
                    </div>

                    <div className="bg-gray-200 border-[1px] rounded-lg py-6 px-10 my-16 glass-morphism">
                        <div className="text-xl font-semibold mb-3 range-title">I am sending my shipment on</div>
                    ...
                    <div className=" mb-10 glass-morphism p-2">
                        <div>
                    ...
                    /* <div className="flex flex-col sender-form-container my-8">
                        ...
                        />
                            ...
                            <div className="grid grid-cols-2 gap-5">
                                ...
                                <input disabled={sameAsSender} type="text" Value={sameAsSender ? senderLandmark : ""} className='className="w-full py-2 pl-2 outline-none  rounded-lg' placeholder='Nearest Landmark (optional)' />

                            </div>
                            <label htmlFor="checkbox_same" className='flex items-center gap-x-2 mt-3'>
                                <input onChange={(e) => setSameAsSender(e.target.checked)} type="checkbox" id="checkbox_same" />
                                <h1 className='font-semibold text-lg'>Select pickup address as sender address.</h1>
                            </label>
                            ...
                            <div className="sender-form-container mt-5">
                                <textarea
                                    ...
                                    onChange={e => updateFields({ pickupInstructions: e.target.value })}
